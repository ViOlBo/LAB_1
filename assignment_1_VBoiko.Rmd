...
astronauts |>
  summarise (
    share_female = mean(sex == "female", na.rm = T),
    share_us = mean(nationality == "U.S.", na.rm = T)
  )
...
total_hours_df <- astronauts |>
  group_by(name) |>
  summarise(
  total_hrs_sum = sum(hours_mission, na.rm = T) 
  )
...
ggplot(total_hours_df, aes(x = total_hrs_sum)) +
  ...
  labs(
    ...
    title = "Distribution of Total Mission Hours"
  )
```
...
share_civilians <- astronauts |>
  group_by(year_of_mission) |>
  summarise (
    share_civilians = mean(military_civilian == "civilian", na.rm = T)
  ) 
ggplot(share_civilians, aes(x = year_of_mission, y = share_civilians)) +
  geom_line() +
  geom_point() +
  labs(
    x = "Year of mission",
    y = "Share of civilian astronauts",
    title = "Share of Civilian Astronauts by Mission Year"
  )
...
share_civilians <- astronauts |>
  ...
  labs(
    title = "Overall Share of Civilian Astronauts by Year of Mission",
    x = "Year of Mission",
    y = "Share of Civilian Astronauts"
  ) +
  theme_minimal()
...
get_ages <- function(year_of_birth) {
  current_year <- as.numeric(format(Sys.Date(), "%Y"))
  current_year - year_of_birth
}

#  test:
get_ages(c(1934, 1935, 1921))
...
check_password <- function(pw) {
  ...
  if (str_detect(pw, "[a-zA-Z]") && str_detect(pw, "[0-9]") && str_detect(pw, "[!@#$%^&*?]")) {
    return("Strong")
  }
  
  if (str_detect(pw, "[a-zA-Z]") && str_detect(pw, "[0-9]")) {
    return("Not bad")
  }
...
passwords <- c("PassWORD123", "abc12", "SecretPass", "Secret123", "Secret123!")
 
sapply(passwords, check_password)
